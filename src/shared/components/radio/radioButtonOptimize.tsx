...
import { Provider, Consumer, } from './radioContext'
...
export default class RadioButtonComponent extends Component<propsTypes, stateTypes> {
                ...
                context => (
                    <View style={{ opacity: this.props.disabled ? 0.4 : 1 }}>
                        <TouchableWithoutFeedback
                            ...
                            onPress={() => context.onSelect(this.props.index!, this.props.value)}
                        >
                            <View style={[
                                styles.container, 
                                this.props.style, 
                                this.props.isSelected ? { backgroundColor: context.highlightColor, } : null
                            ]}>
                                {/* radio圆点 */}
                                <View style={[styles.radio, this.getRadioStyle(context)]}>
                                    {this.isSelectedRadio(context)}
                                </View>
                                {/* radio圆点后的内容 */}
     ...
     * 单选按钮的形状定义
     */
    private getRadioStyle = (context: any) => {
        return {
            width: context.size,
            height: context.size,
            borderRadius: context.size / 2,    // 呈现出圆角，因此是size的一半
            borderWidth: context.thickness,
            borderColor: this.props.isSelected && context.activeColor ? context.activeColor : context.color
        }
    }
     ...
     * 被选中时的单选按钮形态
     */
    private isSelectedRadio = (context: any) => {
        if(this.props.isSelected) {
            return <View style={this.getRadioDotStyle(context)} />
        }        
    }
     ...
     * 单选按钮中被选中时中间那个实心圆点
     */
    private getRadioDotStyle = (context: any) => ({
        width: context.size / 2,
        height: context.size / 2,
        borderRadius: context.size / 4,
        backgroundColor: this.props.isSelected && context.activeColor ? context.activeColor : context.color
    })
